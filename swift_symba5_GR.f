...
c
c  This version includes GR precession using the potential of Nobili &
c  Roxburgh (1986).
c  The adopted speed of light assumes that the units of length and time
c  are AU and and years, respectively. Then the unit of mass is such
c  that G M = 4 pi^2 for one solar mass.
c     
...
c...  Executable code 
      ...
      c = 2.99792458d10*365.25d0*24.d0*3600.d0/1.495978707d13
...
c Apply kick to each planet's velocity due to GR precession potential by
c half dt
         call coord_h2b(nbod,mass,xh,yh,zh,vxh,vyh,vzh,
     &                  xb,yb,zb,vxb,vyb,vzb,msys)
         do i=2,nbod
            grfac =  3.0d0*(mass(1)/c/(xb(i)**2+yb(i)**2+zb(i)**2))**2
            vxb(i) = vxb(i) - grfac*xb(i)*dt
            vyb(i) = vyb(i) - grfac*yb(i)*dt
            vzb(i) = vzb(i) - grfac*zb(i)*dt
         enddo
